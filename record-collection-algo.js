...
function updateRecords(records, id, prop, value) {
  // The goal is to update a record into the records collection object

  // If value is an empty string, delete the given prop from the album
   if(value === ""){
     delete records[id][prop];
   } 

     
  // If prop isn't tracks (because it's an array) and value isn't an empty string, assign the value       to that album's prop
   else if(prop != "tracks" && value != ""){
     records[id][prop] = value;
   }
     
  // First, if the album does not have a tracks property, assign it an empty array. Then add the         value as the last item in the album's tracks array. (We will use the push method to add into         an empty array for the tracks)
